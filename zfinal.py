...
while True:
    ...
    if current_time - last_process_time >= PROCESS_EVERY_SECONDS:
        ...
        if keypoints is not None and keypoints.shape[0] > 0:
            ...
            #         passing, min_dist = detect_object_passing(person1, person2, pid1, pid2)
            #         if passing:
            #             crop_and_save_alert(frame, person1, pid1, f"passing_{pid2}")
            #             crop_and_save_alert(frame, person2, pid2, f"passing_{pid1}")
            #             scoreboard[pid1] += 1
            #             scoreboard[pid2] += 1
            #             print(f"🔁 Object passing detected between {pid1} and {pid2} (distance: {min_dist:.1f})")
            ...
            for i, person in enumerate(keypoints):
                ...
                if left_shoulder[2] > 0.4 and right_shoulder[2] > 0.4:
                    ...
                    if tracked_persons[pid]["baseline_x"] is None:
                        tracked_persons[pid]["baseline_x"] = shoulder_center_x
                    else:
                        delta_x = abs(shoulder_center_x - tracked_persons[pid]["baseline_x"])
                        if "last_horizontal_alert" not in tracked_persons[pid]:
                            tracked_persons[pid]["last_horizontal_alert"] = None

                        # Check movement and cooldown
                        if delta_x > 50:
                            last_alert = tracked_persons[pid]["last_horizontal_alert"]
            ...
            for pid, data in tracked_persons.items():
                if data["disappear_start_time"] is None:
                    ...
                    crop_and_save_alert(frame, frame, pid, "disappear")  # Use full frame since no person crop
                    scoreboard[pid] += 1
                    data["disappeared"] = True
